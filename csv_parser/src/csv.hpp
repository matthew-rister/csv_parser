...
template <typename ColumnType, typename TupleType, std::size_t TupleSize>
struct Index {
...
template <typename ColumnType, typename TupleType>
struct Index<ColumnType, TupleType, 0> {
...
namespace csv {
	...
	public:
		...
		ColumnType get(const std::size_t i, const std::size_t j) {
			return get2<ColumnType>(entries_[i], j);
		}

		template <typename ColumnType, class Tuple>
		static ColumnType get2(const Tuple& tuple, const std::size_t i) {
